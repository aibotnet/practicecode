...
def Browse_Category_Rest(category, j, f):
    ...
    while(i <= total_store_count_on_page) :
            ...
            store_name = store_name.encode('ascii', 'ignore')
            f.write(store_name+"|"+str(count))
            #k=0
            if count>=1 :
                for k in range(0, count):
                    f.write("|"+str(merchant_info[k]))
                #print "x"
...
def main():
    f=open('/home/vkthakur/Desktop/bizrate_data/bizrate_text_finalmerchantinfo4.txt', "w")

    #departments = ['jewelry-watches', 'mature', 'musical-instruments-accessories', 'office-supplies', 'pet-supplies',
    #               'ports-equipment-outdoor-gear',   'toys-games', 'video-games-accessories']

    departments = ['sports-equipment-outdoor-gear',   'toys-games', 'video-games-accessories']
